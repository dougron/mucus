...
public class MuG_EscapeTone_RRP implements MuGenerator
{
	...
	public void generate()
	{
		...
		embellishment.addTag(MuTag.IS_ESCAPE_TONE);
	...
	{
		int contour = -1;
		dealWithContourAndMakeNote(mu, previousMu, contour, embellishmentMu, lengthInQuarters);
	}
	...
	{
		int pitch = getNextDiatonicNote(aPitch, noteVector, embellishmentMu);
	...
	{
		return new MuG_EscapeTone_RRP(relativeRhythmicPosition, escapeToneType, accentType);
	}
	...
	public static MuGenerator getMuGeneratorFromXMLElement(Element element)
	{
		...
		return new MuG_EscapeTone_RRP(rrp, ett, at, ntt);
	}
}
